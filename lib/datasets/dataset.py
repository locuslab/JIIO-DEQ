...
class SinusoidDataset(Dataset):
    ...
    def generate_tasks(self, num_tasks=None):
        ...
        for i in range(num_tasks):
            amp = 1#np.random.uniform(low=self.amp_range[0], high=self.amp_range[1])
            ...
            x_val = np.random.uniform(low=self.input_range[0], high=self.input_range[1], size=self.nval).reshape(-1, 1, 1, 1)
            ...
            if self.float16:
                task = dict(amp=amp,
                            phase=phase,
                            x_train=to_device(x_train, self.use_gpu).half(),
                            y_train=to_device(y_train, self.use_gpu).half(),
                            x_val=to_device(x_val, self.use_gpu).half(),
                            y_val=to_device(y_val, self.use_gpu).half(),
                            x_all=to_device(x_all, self.use_gpu).half(),
                            y_all=to_device(y_all, self.use_gpu).half(),
                            )
            else:
                task = dict(amp=amp,
                            phase=phase,
                            x_train=to_device(x_train, self.use_gpu),
                            y_train=to_device(y_train, self.use_gpu),
                            x_val=to_device(x_val, self.use_gpu),
                            y_val=to_device(y_val, self.use_gpu),
                            x_all=to_device(x_all, self.use_gpu),
                            y_all=to_device(y_all, self.use_gpu),
                            )
...
class OmniglotTask(object):
    ...
    def __init__(self, train_val_permutation, root=DATA_DIR, num_cls=5, num_inst=1, train=True):
        ...
        for c in classes:
            ...
            temp = [os.path.join(c, x.decode('ascii')) for x in os.listdir(c)]
...
class OmniglotFewShotDataset(Dataset):
    ...
    def __init__(self, task_defs, transform=None, target_transform=None, img_size=28, GPU=False):
    ...
    def __getitem__(self, index):
        ...
        for idx, file_name in enumerate(task_def.train_ids):
            ...
            x_train.append(image.reshape(1, self.img_size, self.img_size)/255.0)
        ...
        for idx, file_name in enumerate(task_def.val_ids):
            ...
            image = np.array(image, dtype=np.float32)
            image = rotate(image, rotations[idx // task_def.num_inst])
            x_val.append(image.reshape(1, self.img_size, self.img_size)/255.0)
            y_val.append(task_def.val_labels[idx])
            # break


        # base transforms
        x_train = to_device(np.array(x_train), self.use_gpu)
        y_train = to_device(np.array(y_train), self.use_gpu)
        x_val   = to_device(np.array(x_val), self.use_gpu)
        y_val   = to_device(np.array(y_val), self.use_gpu)
        ...
        task = dict(x_train=x_train,
                    y_train=y_train.long(),
                    x_val=x_val,
                    y_val=y_val.long())

        return task# x_train, y_train.long()#, x_val, y_val.long()
